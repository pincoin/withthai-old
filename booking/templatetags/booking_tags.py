...
@register.simple_tag
def get_areas():
    cache_key = 'golf.templatetags.booking_tags.get_areas()'
...
def get_provinces(slug):
    cache_key = 'golf.templatetags.booking_tags.get_areas({})'.format(slug)
    cache_time = settings.CACHES['default']['TIMEOUT_DAY']
...
def get_golf_club_list(clublist_code):
    cache_key = 'golf.templatetags.booking_tags.get_golf_club_list({})'.format(clublist_code)
    cache_time = settings.CACHES['default']['TIMEOUT_DAY']
